...
export default function Order(props) {
  ...
  }, []) // OrderItem.js 에서 처리한 값이 있을 경우 최신화
  const refresh = () => {
          ...
          <Container>
            <Row className="row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
              {Object.keys(orders).map(key =>
                <Col key={key}>
                  <OrderItem orders={orders[key]} setOrders={setOrders} list={orders} id={key} setShowModal={setShowModal} setData={setData} deleteModal={deleteModal} orderCount={orderCount} setOrderCount={setOrderCount} />
                </Col>
              )}
            </Row>
          </Container>
